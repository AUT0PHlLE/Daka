...
SCAN_DAYS = 60  # Scan last 60 days
...
def register_handlers(app, all_apps=None):
    ...
    async def devour_menu(client, message):
        ...
        if state["step"] == "main_menu":
            ...
            # 1) Scan all users (text1) - MODIFIED: Last 60 days only, no service messages
            if opt == "1":
                state["step"] = "scanning"
                status = await message.reply(f"🔍 Scanning users from last {SCAN_DAYS} days in **{state['target_name']}**...")
                user_msgs = {}
                cutoff_time = datetime.now() - timedelta(days=SCAN_DAYS)
                try:
                    async for msg in client.get_chat_history(chat_id):
                        # Check if message is within last 60 days
                        ...
                        # Only collect real text messages from actual users
                        if (
                            msg.from_user
                            and not msg.from_user.is_bot
                            and not getattr(msg.from_user, "is_deleted", False)
                            and msg.text  # Ensure it has text content
                        ):
                ...
                await status.edit(
                    f"✅ Found **{last_count}** users (last {SCAN_DAYS} days, real messages only).\n\nHow many to execute (1-{last_count})? Reply with a number."
                )
                return
