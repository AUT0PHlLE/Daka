...
def register_handlers(app, all_apps=None):
    ...
    async def devour_menu(client, message):
        ...
        if state["step"] == "main_menu":
            ...
            # 1) Scan all users (1 message per user, skip joins/leaves, deleted accounts, own messages, configurable days)
            if opt == "1":
                state["step"] = "scanning"
                status = await message.reply(f"🔍 Scanning all users in **{state['target_name']}** (last {SCANNING_DAYS} days, 1 message per user)...")
                user_msgs = {}
                cutoff_date = datetime.utcnow() - timedelta(days=SCANNING_DAYS)
                try:
                    async for msg in client.get_chat_history(chat_id):
                        # Filter: Only messages within last SCANNING_DAYS
                        ...
                        if msg.from_user and getattr(msg.from_user, "is_deleted", False):
                            continue
                        ...
                        # Store only 1 message per user (first one found)
